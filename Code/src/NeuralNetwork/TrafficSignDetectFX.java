...
public class TrafficSignDetectFX
{
	...
	public TrafficSignDetectFX() {
		webSource = new VideoCapture("http://172.20.10.7:32805/video"); /*Sets where to get footage from "0":front facing cam
		...
		frame = new Mat();
		mem = new MatOfByte();
		signDetect = new CascadeClassifier("C:/opencv/build/etc/haarcascades/stopsign_classifier.xml");//Loads classifier
				...
				while (runnable) {
					if (webSource.grab()) {
						try {
							...
							signDetect.detectMultiScale(frame, signDetections, 1.05, 4, 0, new Size(30, 30), new Size());
